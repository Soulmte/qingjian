...
* **库比程序新的时候不要去搬库。** 回退到旧版本（或装了旧安装包）再启动，库里会有当前程序不认识的迁移，sqlx 默认因此拒绝启动（`MigrateError::VersionMissing`）。这不是坏库：`init_pool_or_recover` 会按老的一套把它打开并提示更新，而不是隔离——隔离等于把设置、工作区、笔记索引一起丢掉，那三样都不在 `.md` 里，重建不回来。

* **排查「打不开」先对一下版本。** 0.1.8 撞上 0.1.9 建的库就是这么炸的：迁移被拒绝 → 那一刻连接池还没关 → 隔离改名撞上「另一个程序正在使用此文件」（os error 32，而占用它的正是青简自己）→ 弹一句「无法打开工作区数据库」接着直接退出，看起来像崩溃。现在迁移失败时会先关池再报错，改名也带重试；那句错误里也不再丢掉真正的第一个原因。
