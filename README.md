...
<p>安装包没有代码签名（个人项目买证书一年上千元），所以浏览器的下载保护会拦它，提示「不常见的文件，可能有害」或干脆阻止。它拦的是「没签名 + 下载量还少」这两件事，不是文件里真有东西。Gitee 那边还多一层：下载地址会重定向到它的共享附件域名，那是个带一长串 token 的地址，在浏览器看来更可疑。</p>

<p>三条出路，挑一条：</p>

<ul>
<li><strong>在拦截提示里选「保留」/「继续下载」。</strong>Edge 一般要点开提示面板的「…」才有这一项。</li>
<li><strong>让青简自己去下。</strong>「设置 → 关于 → 更新」里点检查更新，更新包是程序自己下的，不经过浏览器，不会被拦。</li>
...
<li>要同步发到 Gitee（账号 <code>rain-drops</code>，与 GitHub 的 <code>Soulmte</code> 不同），把 CI 已经建好的安装包与 <code>latest.json</code> 取到本地再上传即可，不必本地重新构建：<code>python scripts/publish.py gitee --repo rain-drops/qingjian --tag v0.2.0 --notes-file release-notes.md --create-repo --latest-json &lt;CI 生成的 latest.json&gt; --asset "本地路径=ASCII 发布名"</code>。<strong>自动更新先问 Gitee、问不到再问 GitHub</strong>，所以这一步不能省：Gitee 那边要有一份下载地址指向自己的 <code>latest.json</code>，细节见 <code>CONTRIBUTING.md</code>。</li>
</ol>
...
<li>自动更新只在 Windows 上启用（NSIS）。更新包下载到系统临时目录，青简在下次启动时会把这类残留清理掉，既不会留在「下载」文件夹，也不会在临时目录里越积越多。</li>
<li><strong>安装包没有代码签名。</strong>所以浏览器下载保护和 SmartScreen 会拦它、提示「未知发布者」，Windows 上首次运行可能还要点一下「仍要运行」。这是「无签名 + 下载量少」，不是文件有问题。要彻底消掉这个提示，得买一张代码签名证书（OV 约一年上千元），或申请面向开源项目的免费签名服务。</li>
...
<li>Gitee：<a href="https://gitee.com/rain-drops/qingjian">https://gitee.com/rain-drops/qingjian</a>（账号名与 GitHub 不同；自动更新优先走这里）</li>
</ul>
